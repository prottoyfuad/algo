...
/// PurpleCrayon - cf 1416F - showing off
/// https://codeforces.com/contest/1416/submission/119101623
...
class LRDinic {
 ...
 public:
  ...
  long long compute(const int s, const int t) {
    ...
    if (flow_aux != total_demand) return 0;
    // Remove extra edges
    while (extra_edges--) {
...
vector<pair<int, int>> solve_match(int L, int R, vector<pair<int, int>> edges, vector<int> left, vector<int> right) {
  ...
  for (int i = 0; i < L; i++) {
    flower.add_arc(S, i, needL[i], 1);
  }
  for (auto e : edges) {
    flower.add_arc(e.first, L+e.second, 0, 1);
  }
  for (int i = 0; i < R; i++) {
    flower.add_arc(L+i, T, needR[i], 1);
  }
...
/*
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (cyc[i][j] == make_pair(-1, -1)) {
      ...
      for (int k = 0; k < 4; k++) {
        int ni = i+dx[k], nj = j+dy[k];
        if (min(ni, nj) < 0 || min(n - ni, m - nj) <= 0 || make_pair(ni, nj) != cyc[i][j]) continue;
          dir[i][j] = dc[k];
          break;
        }
      }
