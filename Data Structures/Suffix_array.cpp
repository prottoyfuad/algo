...
/// O (n.logn.logn) // almost brute force :p
...
vector<int> suffix_array1(const string& s) {
  int n = s.length() + 1;
  ...
  for (int k = 1; k < n; k <<= 1) {
    ...
    for (int i = 1; i < n; i++) {
      delta += rank[order[i]] != rank[order[i - 1]] || rank[add(order[i], k)] != rank[add(order[i - 1], k)];
